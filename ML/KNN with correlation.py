...
print("PKM2", Counter(Y_PKM2))
print("ERK2", Counter(Y_ERK2))
...
"""
# characteristics importance (Visualization)
result = permutation_importance(best_model_PKM2, X_test_PKM2_scaled, Y_test_PKM2, n_repeats=30, random_state=42, n_jobs=2)
perm_importances = pd.Series(result.importances_mean, index=X_PKM2.columns)

# Soglia di importanza
threshold = 0.004
important_features = perm_importances[perm_importances > threshold]

plt.figure(figsize=(10, 6))
important_features.plot(kind='barh', color='skyblue')
plt.xlabel('Mean importance per permutation')
plt.ylabel('Characteristics')
plt.title('PKM2: important characteristics')
plt.show()
"""
...
"""
# characteristics importance (Visualization)
result = permutation_importance(best_model_ERK2, X_test_ERK2_scaled, Y_test_ERK2, n_repeats=30, random_state=42, n_jobs=2)
perm_importances = pd.Series(result.importances_mean, index=X_ERK2.columns)
...
plt.xlabel('Mean importance per permutation')
plt.ylabel('Characteristics')
plt.title('ERK2: important characteristics')
plt.show()
"""
...
# PKM2
results = pd.DataFrame(X_test_PKM2_scaled)
results['Predicted'] = Y_pred_test_PKM2
results['True'] = Y_test_PKM2.values
num_plots = 4
feature_combinations = np.random.choice(results.columns[:-2], size=(num_plots, 2), replace=False)
plt.figure(figsize=(15, 12))
for i, (feat1, feat2) in enumerate(feature_combinations):
    ax = plt.subplot(2, 2, i + 1)
    correct = results[results['True'] == results['Predicted']]
    incorrect = results[results['True'] != results['Predicted']]
    ax.scatter(correct[feat1], correct[feat2], color='green', label='Corretto', alpha=0.6)
    ax.scatter(incorrect[feat1], incorrect[feat2], color='red', label='Errato', alpha=0.6)
    ax.set_xlabel(f'{feat1}')
    ax.set_ylabel(f'{feat2}')
    ax.set_title(f'Scatter Plot di {feat1} vs {feat2}')
    ax.legend()
plt.suptitle("Scatter Plot PKM2", fontsize=16)
plt.tight_layout()
plt.show()

# ERK2
results = pd.DataFrame(X_test_ERK2_scaled)
results['Predicted'] = Y_pred_test_ERK2
results['True'] = Y_test_ERK2.values
num_plots = 4
feature_combinations = np.random.choice(results.columns[:-2], size=(num_plots, 2), replace=False)
plt.figure(figsize=(15, 12))
for i, (feat1, feat2) in enumerate(feature_combinations):
    ...
    ax.set_title(f'Scatter Plot di {feat1} vs {feat2}')
    ax.legend()
plt.suptitle("Scatter Plot ERK2", fontsize=16)
plt.tight_layout()
plt.show()